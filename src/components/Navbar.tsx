...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 w-full shadow-sm bg-warm-ivory/90 backdrop-blur-md border-b border-natural/25">
      ...
      {/* Tier 2: Pills & Search */}
      <div className="border-t border-natural/20 bg-warm-ivory/50">
        <div className="max-w-7xl mx-auto px-4 py-2 sm:px-6 lg:px-8 flex items-center justify-between gap-4 overflow-x-auto overflow-y-hidden no-scrollbar scroll-smooth">
          
          {/* Left Pills */}
          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Clothing Dropdown */}
            <div className="relative group/dropdown">
              <button className="flex items-center gap-1 px-4 py-1.5 bg-white border border-natural/25 hover:border-fern hover:bg-warm-ivory/30 text-fern text-xs font-semibold rounded-full shadow-sm transition-all duration-200">
                Clothing
                <ChevronDown size={14} className="text-natural group-hover/dropdown:text-fern transition-colors" />
              </button>
              {/* Simple dropdown overlay */}
              <div className="absolute left-0 mt-2 w-44 bg-white border border-natural/20 rounded-xl shadow-lg opacity-0 invisible group-hover/dropdown:opacity-100 group-hover/dropdown:visible transition-all duration-200 z-50">
                <div className="py-1 text-xs text-fern font-medium">
                  <a href="#categories" className="block px-4 py-2 hover:bg-warm-ivory hover:text-apricot transition-colors">Dresses & Jumpsuits</a>
                  <a href="#categories" className="block px-4 py-2 hover:bg-warm-ivory hover:text-apricot transition-colors">Linen Shirts & Tops</a>
                  <a href="#categories" className="block px-4 py-2 hover:bg-warm-ivory hover:text-apricot transition-colors">Co-ords & Pants</a>
                  <a href="#categories" className="block px-4 py-2 hover:bg-warm-ivory hover:text-apricot transition-colors">Hand-knit Sweaters</a>
                </div>
              </div>
            </div>
