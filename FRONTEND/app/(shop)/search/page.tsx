  ...
  return (
    <div className=\"group bg-white border border-natural/10 rounded-2xl overflow-hidden shadow-xs hover:shadow-md hover:border-natural/25 hover:-translate-y-0.5 transition-all duration-300 flex flex-col relative\">
      ...
      >
        <Heart
          size={13}
          style={isFav ? { fill: \"#870339\", color: \"#870339\" } : {}}
        />
      </button>
...
function SearchContent() {
  ...
  const handleToggleFavorite = (id: string, name: string, e: React.MouseEvent) => {
    ...
    triggerToast(
      isNow ? `Added ${name} to Wishlist! ❤️` : `Removed ${name} from Wishlist.`
    );
  };
        ...
        <div className=\"flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5\">
          ...
          <div className=\"flex items-center gap-3\">
            ...
            <p className=\"text-xs text-[#4a556a]/65 font-medium\">
              {sortedProducts.length === 0 ? (
                ...
                <>
                  <span className=\"font-bold text-[#4a556a]\">
                    1–{sortedProducts.length}
                  </span>{\" \"}
                  of{\" \"}
                  <span className=\"font-bold text-[#4a556a]\">{sortedProducts.length}</span>{\" \"}
                  results
                </>
              )}{\" \"}
              {query && (
                <>
                  for{\" \"}
                  <span className=\"text-apricot font-bold\">&ldquo;{query}&rdquo;</span>
                </>
              )}
            </p>
          </div>
          ...
          <div className=\"flex items-center gap-2 ml-11 sm:ml-0 flex-shrink-0\">
            ...
            <div className=\"relative flex items-center gap-1.5 bg-white border border-natural/20 rounded-full pl-3 pr-8 py-1.5 shadow-xs\">
              ...
              <ChevronDown
                size={12}
                className=\"absolute right-3 pointer-events-none text-[#4a556a]/40\"
              />
            </div>
          </div>
        </div>
